const state = {
	// 当前视频源
	siteKey: {
			id: 2,
			key: "88zyw",
			name: "88 影视资源站",
			api: "http://www.88zyw.net/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
	// 视频源
	siteList: [{
			id: 1,
			key: "mahuazy",
			name: "麻花资源",
			api: "https://www.mhapi123.com/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 2,
			key: "88zyw",
			name: "88 影视资源站",
			api: "http://www.88zyw.net/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 3,
			key: "apibdzy",
			name: "百度云资源",
			api: "https://api.apibdzy.com/api.php/provide/vod/at/xml",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 4,
			key: "mbo",
			name: "秒播资源",
			api: "http://caiji.mb77.vip/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 5,
			key: "zuidazy",
			name: "最大资源网",
			api: "http://www.zdziyuan.com/inc/api.php",
			download: "http://www.zdziyuan.com/inc/apidown.php",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 6,
			key: "123ku",
			name: "123 资源",
			api: "http://cj.123ku2.com:12315/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 7,
			key: "okzy",
			name: "OK 资源网",
			api: "http://cj.okzy.tv/inc/api.php",
			download: "http://cj.okzy.tv/inc/apidown.php",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 8,
			key: "kuyunzy",
			name: "酷云资源",
			api: "http://caiji.kuyun98.com/inc/ldg_api.php",
			download: "http://caiji.kuyun98.com/inc/apidown.php",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 9,
			key: "kubozy",
			name: "酷播资源",
			api: "http://api.kbzyapi.com/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 10,
			key: "yongjiuzy",
			name: "永久资源",
			api: "http://cj.yongjiuzyw.com/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 11,
			key: "rrzy",
			name: "人人资源",
			api: "http://www.rrzyw.cc/api.php/provide/vod/from/rrm3u8/at/xml/",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 12,
			key: "bbkdj",
			name: "步步高顶尖资源网",
			api: "http://api.bbkdj.com/api",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 13,
			key: "zuixinzy",
			name: "最新资源",
			api: "http://api.zuixinapi.com/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 14,
			key: "subo988",
			name: "速播资源站",
			api: "https://www.subo988.com/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		},
		{
			id: 15,
			key: "1886zy",
			name: "1886 资源",
			api: "http://cj.1886zy.co/inc/api.php",
			download: "",
			group: "默认",
			isActive: true,
			status: "可用"
		}
	]
}
const mutations = {
	// 更改当前视频源地址key
	SET_SITEKEY(state, payload) {
		state.siteKey = payload
	}
}

const actions = {
	// 获取当前视频源
	async getSiteKeyAsync({
		state
	}, key) {
		// state.siteKey[key] '/web'
		return '/web'
	},
	// 设置当前视频源地址key
	async changeSiteKeyAsync({
		commit
	}, data) {
		commit('SET_SITEKEY', data)
		return data
	}
}

export default {
	namespaced: true,
	state,
	mutations,
	actions
}
